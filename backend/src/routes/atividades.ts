...
export const getProjectParticipants = async (projectId: number) => {
  const [participants] = await pool.query<RowDataPacket[]>(`
    ...
    WHERE pp.id_projeto = ?
  `, [projectId]);
  return participants.map((p: RowDataPacket) => p.email_usuario);
};
  ...
  try {
    ...
    if (participantes && participantes.length > 0) {
      ...
      const [users] = await pool.query<RowDataPacket[]>(
        `SELECT id_usuario FROM usuarios WHERE email_usuario IN (?)`,
        [participantes]
      );
      ...
      const responsaveisValues = users.map((user: RowDataPacket) => [idAtividade, user.id_usuario]);
      await pool.query(
  ...
  export const deletarAtividade = async (req: Request, res: Response) => {
    try {
      ...
      const [atividade] = await pool.query<RowDataPacket[]>(
        `SELECT id_atividade FROM projetos_atividades WHERE id_atividade = ?`,
        [id]
      );
