...
interface StoryPointsResult extends RowDataPacket {
  status: string;
  total_storypoints: number;
}

export const getStoryPointsByStatus = async (req: Request, res: Response) => {
  const { id } = req.params;
  const connection = await pool.getConnection();

  try {
    // Verifica se o projeto existe primeiro
    const [projeto] = await connection.query<RowDataPacket[]>(
