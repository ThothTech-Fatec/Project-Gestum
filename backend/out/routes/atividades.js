...
const obterOrcamentoProjeto = async (connection, projetoId) => {
    const [orcamento] = await connection.query(`SELECT valor FROM orcamento WHERE id_projeto = ?`, [projetoId]);
    return orcamento.length > 0 ? orcamento[0].valor : 0;
};
// Função para obter orçamento total utilizado pelas atividades do projeto
const obterOrcamentoUtilizado = async (connection, projetoId) => {
...
const validarOrcamentoAtividade = async (connection, projetoId, orcamentoAtividade, atividadeId) => {
    ...
    if (atividadeId) {
        const [orcamentoAtual] = await connection.query(`SELECT valor FROM orcamento_ati WHERE id_atividade = ?`, [atividadeId]);
...
export const obterResumoOrcamento = async (req, res) => {
    try {
...
export const criarAtividade = async (req, res) => {
    try {
        const { id_projeto, nome_atividade, descricao_atividade, storypoint_atividade, participantes, isResponsavel, inicio_atividade, fim_atividade, data_limite_atividade, userId, orcamento } = req.body;
        ...
        try {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            ...
            if (orcamento && orcamento > 0) {
                const [orcamentoProjeto] = await connection.query(`SELECT id_orcamento FROM orcamento WHERE id_projeto = ?`, [id_projeto]);
                let idOrcamento;
                if (orcamentoProjeto.length === 0) {
                    const [resultOrcamento] = await connection.query(`INSERT INTO orcamento (id_projeto, valor) VALUES (?, ?)`, [id_projeto, 0]);
                    idOrcamento = resultOrcamento.insertId;
                }
                else {
                    idOrcamento = orcamentoProjeto[0].id_orcamento;
                }
                await connection.query(`INSERT INTO orcamento_ati (valor, id_atividade, id_orcamento) VALUES (?, ?, ?)`, [orcamento, idAtividade, idOrcamento]);
            }
...
export const atualizarAtividade = async (req, res) => {
    try {
        ...
        const { nome_atividade, descricao_atividade, storypoint_atividade, participantes, isResponsavel, userId, orcamento } = req.body;
        if (!isResponsavel) {
            return res.status(403).json({
                success: false,
        ...
        try {
         ...
         WHERE id_atividade = ?`, [
            ...
            if (orcamento !== undefined) {
                const [orcamentoExistente] = await connection.query(`SELECT id_orcamento_ati FROM orcamento_ati WHERE id_atividade = ?`, [activityId]);
                const [orcamentoProjeto] = await connection.query(`SELECT id_orcamento FROM orcamento WHERE id_projeto = ?`, [projetoId]);
                if (orcamentoProjeto.length === 0) {
                    if (orcamento > 0) {
                        const [resultOrcamento] = await connection.query(`INSERT INTO orcamento (id_projeto, valor) VALUES (?, ?)`, [projetoId, 0]);
                        const idOrcamento = resultOrcamento.insertId;
                        await connection.query(`INSERT INTO orcamento_ati (valor, id_atividade, id_orcamento) VALUES (?, ?, ?)`, [orcamento, activityId, idOrcamento]);
                    }
                }
                else {
                    ...
                    if (orcamentoExistente.length > 0) {
                        if (orcamento > 0) {
                            await connection.query(`UPDATE orcamento_ati SET valor = ? WHERE id_atividade = ?`, [orcamento, activityId]);
                        }
                        else {
                            await connection.query(`DELETE FROM orcamento_ati WHERE id_atividade = ?`, [activityId]);
                        }
                    }
                    else if (orcamento > 0) {
                        await connection.query(`INSERT INTO orcamento_ati (valor, id_atividade, id_orcamento) VALUES (?, ?, ?)`, [orcamento, activityId, idOrcamento]);
            ...
            if (participantes && Array.isArray(participantes)) {
                ...
                if (participantes.length > 0) {
                    ...
                    if (novosResponsaveis.length > 0) {
                        ...
                        await criarNotificacao(NOTIFICATION_TYPES.RESPONSAVEL_ADICIONADO, `${nomeEditor} adicionou ${nomesResponsaveis} como responsáveis pela atividade "${nomeAtividade}"`, projetoId, null, activityId, 'atividade');
                    }
                }
            }
            await criarNotificacao(NOTIFICATION_TYPES.ATIVIDADE_ATUALIZADA, `${nomeEditor} atualizou a atividade "${nomeAtividade}"`, projetoId, null, activityId, 'atividade');
            await connection.commit();
            return res.json({
                success: true,
...
export const obterParticipantesProjeto = async (req, res) => {
    try {
        ...
        const connection = await pool.getConnection();
        try {
            const [participantes] = await connection.query(`SELECT 
           u.id_usuario, 
           u.email_usuario, 
           u.nome_usuario,
           pp.tipo
